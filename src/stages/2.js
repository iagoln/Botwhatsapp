...
async function execute(user, msg) {
  ...
  for (let i = 0; i < eventosfinal.length; i++) {
    ...
    if ((Date.parse(diaconsulta) - (frases.solicitareagedamento * 60000)) > Date.now()) {
      ...
      eventosagenda.push(`${frases.reagendamento3.frase1}*${eventosfinal[i].nomecliente}*${frases.reagendamento3.frase2}*${datalimpa}*${frases.reagendamento3.frase3}${eventosfinal[i].linkreagendar} \n`)
    }
  }
  if (eventosagenda == 0) {
